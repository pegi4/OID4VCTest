const type = require("ajv/lib/vocabularies/jtd/type");
...
function installCrypto() {
  ...
  try {
    ...
    console.log('Error importing react-native-securerandom or expo-crypto', e);
  }
        ...
        crypto.subtle.importKey = function importKey() {
          ...
          return originalImportKey.apply(this, arguments)
              .then(res => {
                ...
                switch (res.type) {
                  case 'secret':
                    res.usages = res.algorithm.name === 'HMAC' ? ['sign', 'verify']
                        : ['encrypt', 'decrypt'];
                    break;
                  case 'private':
                    res.usages = ['sign'];
                ...
                if (importType === 'jwk' && key.kty === 'RSA') {
                  ...
                  res.algorithm.publicExponent = str2buf.toUint8Array(
                      b64u.toBinaryString(key.e));
                }
                return res;
              });
        }
...
// wrap all methods to ensure they're secure
  ...
  crypto.subtle.generateKey = function generateKey() {
    ...
    if (algo) {
      if (algo.name) {
        algo.name = algo.name.toLowerCase();
      }
      if (algo.hash
          && algo.hash.name) {
        algo.hash.name = algo.hash.name.toLowerCase();
      }
    }
    return originalGenerateKey.apply(this, arguments)
        .then(res => {
          if (res.publicKey) {
            ...
            res.publicKey.algorithm.name = standardizeAlgoName(
                res.publicKey.algorithm.name);
            res.privateKey.usages = ['sign'];
            res.privateKey.algorithm.name = standardizeAlgoName(
                res.privateKey.algorithm.name);
          } else {
            res.algorithm.name = standardizeAlgoName(res.algorithm.name);
            res.usages = res.algorithm.name === 'HMAC' ? ['sign', 'verify']
                : ['encrypt', 'decrypt'];
