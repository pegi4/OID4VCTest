...
export class Agent {
    ...
    schemaValidation;
    context;
    protectedMethods = ['execute', 'availableMethods', 'emit'];
    ...
    constructor(options) {
                ...
                if (plugin?.eventTypes && plugin?.onEvent) {
                    for (const eventType of plugin.eventTypes) {
                        this.eventBus.on(eventType, (args) => {
                            const promise = plugin?.onEvent?.({ type: eventType, data: args }, { ...this.context, agent: this });
        ...
        for (const method of Object.keys(this.methods)) {
            if (!this.protectedMethods.includes(method)) {
                // @ts-ignore
                this[method] = async (args) => this.execute(method, args);
            }
        }
